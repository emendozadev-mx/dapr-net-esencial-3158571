...
namespace WisdomPetMedicine.Rescue.Api.Controllers
{
    ...
    public class IntegrationEventsController: ControllerBase
    {
        ...
        public IntegrationEventsController(IServiceScopeFactory serviceScopeFactory, ILogger<IntegrationEventsController> logger)
        {
        ...
        public async Task<IActionResult> OnPetFlaggedForAdoption(PetFlaggedForAdoptionIntegrationEvent ev)
        {
            ...
            dbContext.RescuedAnimalsMetadata.Add(ev);

            var rescuedAnimal = new RescuedAnimal(RescuedAnimalId.Create(ev.Id));

            await repo.AddRescuedAnimalAsync(rescuedAnimal);

            var message = $"Pet flagged for adoption: {ev.Id}";
